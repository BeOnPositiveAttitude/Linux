...
### Мои заметки по маршрутизации трафика

Если у нас есть доступ к настройке маршрутов на обеих сторонах (в точке A и в точке B), тогда мы можем прописать статические маршруты в точке A и в точке B, и пинги будут успешно ходить.

Важно отметить, что при настройке маршрута командой `ip route add 10.244.1.0/24 via 192.168.56.131`, в качестве шлюза нужно указывать IP-адрес интерфейса соседней машины, с которой мы пытаемся установить связность, а не локальной!

Если же мы пытаемся настроить доступ из namespace в Интернет, например хотим пинговать `8.8.8.8`, то нужно настроить NAT, т.к. у нас нет доступа к серверам Google для настройки обратного маршрута `:)`.

---
---

Включить прослушивание порта можно с помощью netcat: `nc -l -p 8080 -v`. Здесь `-l` - listen, `-p` - port, `-v` - verbose.

Аналогично в namespace: `ip netns exec red nc -l -p 5000 -v`.